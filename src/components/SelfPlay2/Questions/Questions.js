...
const Questions = ({ singleLesson }) => {
  ...
  if (quizEnd) {
    return navigate(
      `result?lessonName=${singleLesson.lessonName}&chapterId=${singleLesson.chapterId}&userId=${userId}&lessonId=${singleLesson.id}&score=${score}&questionsLength=${questions.length}`
    );
  }
